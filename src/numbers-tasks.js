...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const angle = Math.abs(secondAngle - firstAngle);
  return angle;
}
...
function getLastDigit(value) {
  const result = Math.abs(value) % 10;
  return result;
}
...
function isPowerOfTwo(num) {
  const result = Number.isInteger(Math.log2(num)) && num > 0;
  return result;
}
...
function getRandomInteger(min, max) {
  const startNum = Math.ceil(min);
  const endNum = Math.floor(max);
